...
5. **Two-tier clip quality**: while editing, clips are fetched at "preview" quality (360p) — cheap and fast, plenty for on-screen editing. `video_id`/`source_url` are stored per-slot (`ClipState.video_id`, `.source_url`, `.quality`) so that once the user is done editing, a later milestone can re-resolve and download the "full" quality (1080p) version of the exact same video for the final export, without re-searching. Only the preview tier is actually fetched today — the full-quality re-fetch-at-export-time flow itself is deferred to that later milestone.
6. **Output aspect ratio is 16:9 (landscape)**, not the originally-specced 9:16 vertical/phone format — changed per explicit user request. Output canvas is 1920×1080 (`filter_graph.py`'s `OUTPUT_WIDTH`/`OUTPUT_HEIGHT`); crop/zoom math and preview are unaffected by which orientation is chosen. Download cap per source clip is 30s (down from an initial 90s), also per explicit request — still comfortable headroom over any real slot duration.
7. **Lead-in is no longer dropped**: the M3 v1 simplification (export started at the first noun's timestamp, silently cutting the words spoken before it) is fixed — the first slot's clip now extends backward to cover from t=0 of the narration, and the narration audio trim starts at 0 too. No other slot/group's timing is affected (groups concatenate by duration, not absolute time, so extending the first one just pushes everything after it later). Projects also now persist to disk (`.cache/projects/{id}.json`, written on every mutation) so a backend restart mid-session no longer orphans the frontend's in-progress project (previously surfaced as "Project not found" on export).
...
- **M4 — Compositing + crop/zoom + trim-shift UI** ✅ *(delivered; note `proxy.py` turned out unnecessary — preview-quality fetch already provides the low-res material a separate proxy transcode would have)*: `composite_rng.py` assigns each slot a cut/overlay decision at build time (escalating back to a hard cut, capped at 3 stacked layers) and supports independent per-slot re-roll (`POST /api/composite/reroll`) without cascading to other slots. `filter_graph.py` groups consecutive overlay slots into a single rendered "stack" — the base layer's clip is extended to cover the whole stack's duration, each additional layer is time-shifted to its own start and alpha-blended in (50% opacity) via ffmpeg's `overlay` filter, which naturally holds the layer below unchanged until the layer above's shifted PTS arrives (no `enable` gating needed). Crop/zoom is a real ffmpeg `crop` (region + zoom, always followed by a cover-scale safety net to guarantee exact output size) exposed via `PATCH /api/project/{id}/slots/{slot_id}/clip`; the frontend simplifies this to Zoom/Pan-X/Pan-Y sliders (computing the equivalent `crop_rect` before sending) rather than a free-form draggable rectangle, with a CSS-transform approximation for live preview (no ffmpeg round-trip per drag). Trim-window sliding preserves window length server-side (only `trim_start` needs sending; `trim_end` follows automatically). *Verified*: pixel-exact synthetic-clip tests for the overlay alpha-blend math and crop/zoom region math (both matched expected values precisely), plus a full live build→fetch→patch→export run and in-browser slider interaction, both confirmed working.
- **M5 — Per-slot controls** ✅ *(delivered)*: `POST /api/search/next-video` fetches the next `ytsearchN` rank on demand, auto-advancing past any rank whose `video_id` is already in `seen_video_ids` (up to 5 attempts) so it never re-shows a clip already seen for that slot. `POST /api/search/new-search` clears `manual_override`, cycles `active_index` to the next of the ~10 candidates (wrapping around), resets `results`, and fetches its top result. `POST /api/search/manual-search` takes free text, sets it as `manual_override`, and fetches its top result the same way. All three share an `_apply_fetch_result` helper and persist search-string changes to disk even if the fetch itself fails (so a retry doesn't re-attempt an already-abandoned query), and none of them touch `start_time`/`duration` — verified live that swapping a slot's clip via all three controls repeatedly never shifts any slot's timing. Frontend shows the currently-active search string per slot plus the three controls, with per-slot loading/error state (a failed swap shows the real reason inline, e.g. no unseen result found, without disturbing the slot's existing clip). "Generate more candidates" beyond the initial ~10 was **not** built — out of scope for the "done when" criteria above; cycling wraps back to candidate 0 instead.
- **M6 — Audio** ✅ *(delivered)*: the `amix`-based mixing and per-clip `volume`/`muted` handling already existed in `filter_graph.py` from M3/M4 — M6's actual work was the missing plumbing: `PATCH /api/project/{id}/narration` (global mute) plus a Zoom-style Volume slider and Mute checkbox per clip in the editor, wired to the existing `patchSlotClip` endpoint. *Verified precisely* via `ffmpeg`'s `volumedetect` filter, not just by ear: narration-muted export measured -91dB (true silence) vs -32dB with it unmuted; a clip's volume set to 0.2 measured -14.2dB quieter than the same clip at 1.0 (predicted: 20·log₁₀(0.2) ≈ -14.0dB) — confirms real proportional gain, not a binary toggle. Along the way, found and fixed a real gap from M5: a slot whose fetch failed had no UI at all (the editor list only rendered "ready" slots), leaving the user stuck with no way to retry it — failed slots now get a reduced card (noun + error + the next-video/new-search/manual-search controls, extracted into a shared `SlotSearchControls` component used by both card variants), verified live by recovering a real failed fetch through the UI.
